...
public class RepasoHotel {
    ...
    public static Map<String, Double> calcularFacturas(Map<String, Map<Integer, Integer>> registros, Map<Integer, Double> precios) {
        ...
        for (String dni : registros.keySet()) {
            ...
            Map<Integer, Integer> habitaciones = registros.get(dni);
            for (Integer hab : habitaciones.keySet()) {
                Double pre = precios.getOrDefault(hab, 0d);
                double importe = pre * habitaciones.get(hab);
                total += importe;
            }
    ...
    public static Map<Integer, Integer> calcularOcupacionPorHabitacion(Map<String, Map<Integer, Integer>> registros) {
        ...
        for(String dni : registros.keySet()) {
            Map<Integer, Integer> habitaciones = registros.get(dni);
            for (Integer habitacion : habitaciones.keySet()){
                Integer noches = habitaciones.get(habitacion);

                if(ranking.containsKey(habitacion)) {
                    ranking.put(habitacion, ranking.get(habitacion));
                } else {
